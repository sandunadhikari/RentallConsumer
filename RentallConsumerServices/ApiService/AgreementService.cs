...
namespace RentallConsumerServices.ApiService
{
    public class AgreementService
    {
        ...
        public AddChecklistMobileResponse saveCheckListMobile(AddChecklistMobileRequest addChecklistMobileRequest, string token)
        {
            AddChecklistMobileResponse result = null;
            try
            {
                using (var client = new HttpClient())
                {
                    client.BaseAddress = new Uri(ConstantData.ApiURL.ToString() + "CheckListMobile/AddChecklist");
                    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                    client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);

                    var myContent = JsonConvert.SerializeObject(addChecklistMobileRequest);
                    var buffer = Encoding.UTF8.GetBytes(myContent);
                    var byteContent = new ByteArrayContent(buffer);
                    byteContent.Headers.ContentType = new MediaTypeHeaderValue("application/json");
                    ...
                    {
                        var responseStream = response.Content.ReadAsStringAsync().Result;
                        result = JsonConvert.DeserializeObject<AddChecklistMobileResponse>(responseStream);
                    }
                }
            }
