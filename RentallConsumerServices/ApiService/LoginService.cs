    ...
    public class LoginService
    {
        ...
        public int DeleteUserProfile(CustomerLogin loginCustomer, string token)
        {
            ...
            try
            {
                using (var client = new HttpClient())
                {
                    client.BaseAddress = new Uri(ConstantData.ApiURL.ToString() + "Registration/");
                    client.DefaultRequestHeaders.Accept.Clear();
                    client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
                    var body = new List<KeyValuePair<string, string>>
                        {
                            new KeyValuePair<string, string>("customerid", loginCustomer.CustomerId.ToString())
                        };
                    var content = new FormUrlEncodedContent(body);

                    var response = client.DeleteAsync("DeleteUserProfile?customerid=" + loginCustomer.CustomerId.ToString()).Result;
                    if (response.IsSuccessStatusCode)
                    {
                        var responseStream = response.Content.ReadAsStringAsync().Result;
                        resId = JsonConvert.DeserializeObject<int>(responseStream);
                    }
                }
            }
            catch (Exception ex)
