...
namespace RentallConsumer.Popups
{
    ...
    public partial class ExtendPopup : PopupPage
    {
        ...
        private async void updateAgreement()
        {
            ...
            request.extendDate = agreementDetail.CheckinDate.AddDays(int.Parse(NumberOfDays.SelectedItem.ToString())).ToString("MM/dd/yyyy hh:mm tt", CultureInfo.InvariantCulture).Replace("PM", "pm").Replace("AM", "am");

            int agreementID = 0;

            await App.Current.MainPage.DisplayAlert("Alert", "Your agreement no " + agreementDetail.AgreementNumber.ToString() + " will be extended till " + request.extendDate.ToString(), "Ok");
            ...
            //agreementReview.AgreementStatus = AutopreneurClubModel.Constants.AgreementStatusConst.Open;
