...
namespace RentallConsumer.Views
{
    public partial class OtherInformationPage : ContentPage
    {
        ...
        private async void DoneBtn_Clicked(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (!busy)
                    {
                        try
                        {
                            ...
                            await Task.Run(() =>
                            {
                                ...
                                try
                                {
                                    var LExpiryDate = ((DateTime)licenceExpiryDate.Date).ToString("MM/dd/yyyy hh:mm tt", CultureInfo.InvariantCulture);
                                    var expiryDate = DateTime.ParseExact(LExpiryDate, "MM/dd/yyyy hh:mm tt", CultureInfo.InvariantCulture);
                                    customer.LicenseExpiryDate = expiryDate;
                                }
                                catch (Exception ex)
                                {
                                ...
                                else if (licIssueDateSelected)
                                {
                                    try
                                    {
                                        var LIssueDate = ((DateTime)licenceIssueDate.Date).ToString("MM/dd/yyyy hh:mm tt", CultureInfo.InvariantCulture);
                                        var issueDate = DateTime.ParseExact(LIssueDate, "MM/dd/yyyy hh:mm tt", CultureInfo.InvariantCulture);
                                        customer.LicenseIssueDate = issueDate;
                                    }
                                    catch (Exception ex)
                                    {
                    ...
                    {
                        await PopupNavigation.Instance.PushAsync(new Error_popup("Registration failed. Phone number must not be used before. Please try again"));
                    }
        ...
        {
           // licenceNumber.TextColor = (Color)Application.Current.Resources["VeryLightGrayColor"];
        }
        ...
        private void licimageUplaodViewer_Tapped(object sender, EventArgs e)
        {
            ...
            if (licImageFrame.IsVisible)
            {
                ...
                licImageBtn.Source = ImageSource.FromResource("RentallConsumer.Assets.iconWhiteCamera.png", assembly);
            }
            else
            {
                licImageFrame.IsVisible = true;
                licImageBtn.Source = ImageSource.FromResource("RentallConsumer.Assets.iconWhiteUp.png", assembly);
            }
        }
        ...
        void licenceStateEntry_Focused(System.Object sender, Xamarin.Forms.FocusEventArgs e)
        {
            licenceStateEntry.Unfocus();
            Device.BeginInvokeOnMainThread(() => {
                licenceStatePicker.Focus();
            });
        }

        void licenceStatePicker_Unfocused(System.Object sender, Xamarin.Forms.FocusEventArgs e)
        {
            if (licenceStatePicker.SelectedIndex != -1)
            {
                licenceStateEntry.Text = licenceStatePicker.SelectedItem.ToString();
            }
        }
    }
}