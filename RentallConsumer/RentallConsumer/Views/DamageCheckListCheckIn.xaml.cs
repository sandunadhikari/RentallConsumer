...
namespace RentallConsumer.Views
{
    public partial class DamageCheckListCheckIn : ContentPage
    {
        ...
        AddChecklistMobileRequest addChecklistMobileRequest;
        AddChecklistMobileResponse addChecklistMobileResponse;
        private int ansId;
        ...
        public DamageCheckListCheckIn(int agreementId, string AgreementNumber, int VehicleId)
        {
            ...
            addChecklistMobileRequest = new AddChecklistMobileRequest();
                ...
                if (checklistMobileResponse != null)
                {
                    if (checklistMobileResponse.message.ErrorCode == "200")
                    {
                        if (isClick)
                        {
                            ...
                            ansId = checklistMobileResponse.AnswerId;
                        }
                    }
                    else
                    {
        ...
        private async void SaveBtn_Clicked(object sender, EventArgs e)
        {
            ...
            {
                AgreementController agreementController = new AgreementController();
                List<DefaultDamageList> returnListAfterEdited = new List<DefaultDamageList>();
                List<DefaultDamageList> returnListAfterEditedOptional = damageList.ItemsSource as List<DefaultDamageList>;
                List<DefaultDamageList> returnListAfterEditedshortAnswer = damageListNonOption.ItemsSource as List<DefaultDamageList>;
                List<DefaultDamageList> returnListAfterEditedCalender = damageListCalender.ItemsSource as List<DefaultDamageList>;
                if (returnListAfterEditedOptional != null)
                {
                    returnListAfterEdited.AddRange(returnListAfterEditedOptional);
                }
                if (returnListAfterEditedshortAnswer != null)
                {
                    returnListAfterEdited.AddRange(returnListAfterEditedshortAnswer);
                }
                if (returnListAfterEditedCalender != null)
                {
                    returnListAfterEditedCalender.ForEach(m => m.calendarstr = m.Calendar.Date == null ? string.Empty : m.Calendar.Date.ToString());
                    returnListAfterEdited.AddRange(returnListAfterEditedCalender);
                }
                if (returnListAfterEdited != null)
                {
                    addChecklistMobileRequest.AnswerId = ansId;
                    addChecklistMobileRequest.AgreementId = agreementId;
                    addChecklistMobileRequest.DefaultDamageChecklistout = checklistMobileResponse.DefaultDamageChecklistout;
                    addChecklistMobileRequest.DefaultDamageChecklistIn = returnListAfterEdited;//checklistMobileResponse.DefaultDamageChecklistIn;
                    ...
                    if (!busy)
                    {
                        try
                        {
                            ...
                            await Task.Run(async () =>
                            {
                                try
                                {
                                    addChecklistMobileResponse = agreementController.saveCheckListMobile(addChecklistMobileRequest, token);
                                }
                                catch (Exception ex)
                                {
                                    await PopupNavigation.Instance.PushAsync(new ErrorWithClosePagePopup(ex.Message));
                        ...
                        if (addChecklistMobileResponse != null)
                        {
                            if (addChecklistMobileResponse.answerId > 0)
                            {
                                await PopupNavigation.Instance.PushAsync(new SuccessPopUp("Check list details saved successfully.", 0));
                            }
                        }
                    }
                }
