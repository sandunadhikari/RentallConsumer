...
using RentallConsumerController;
...
namespace RentallConsumer.Views
{
    public partial class BookNow : ContentPage
    {
        ...
        protected override async void OnAppearing()
        {
            ...
            TEdropOffLocation.Text = null;
            TEpickUpLocation.Text = null;
            TextEntrydropOff.Text = null;
            TextEntry.Text = null;

            try
            {
                DateTime dateTime1 = DateTime.ParseExact("12:00 am", "hh:mm tt", CultureInfo.InvariantCulture);
                pickUpTime.Time = dateTime1.TimeOfDay;
                dropOfftime.Time = dateTime1.TimeOfDay;
            }
            catch (Exception ex)
            {
                    ...
                    if (getStoreHoursMobileResponse.storeHourModels != null)
                    {
                        if (getStoreHoursMobileResponse.storeHourModels.Count > 0)
                        {
                            ...
                            foreach (SearchStoreHourModel sshm in getStoreHoursMobileResponse.storeHourModels)
                            {
                                ...
                                {
                                    if ((((DateTime)sshm.StartTime).TimeOfDay <= pickUpTime.Time && pickUpTime.Time <= ((DateTime)sshm.EndTime).TimeOfDay) && (((DateTime)sshm.StartTime).TimeOfDay <= dropOfftime.Time && dropOfftime.Time <= ((DateTime)sshm.EndTime).TimeOfDay))
                                    {
                                        var vv = dropOfftime.Time;
                                        if (sshm.IsHoliday)
                                        {
                                    ...
                                    else
                                    {
                                        ...
                                        PopupNavigation.Instance.PushAsync(new Error_popup("Our business hours are " + ((DateTime)sshm.StartTime).ToString("hh:mm tt") + " to " + ((DateTime)sshm.EndTime).ToString("hh:mm tt") + ". Please select a pick up time within those hours."));
                                    }
                                }
        ...
        //        loginIcon.IconImageSource = ImageSource.FromResource("AutopreneurClub.Assets.logOutTool.png", assembly);
        ...
        //            loginIcon.IconImageSource = ImageSource.FromResource("AutopreneurClub.Assets.LogInTool.png", assembly);
        ...
        private void pickUpLocation_Unfocused(object sender, FocusEventArgs e)
        {
            ...
            {
                TEpickUpLocation.Text = pickUpLocation.SelectedItem.ToString();
            }
        }
        ...
        void pickUpTime_Unfocused(System.Object sender, Xamarin.Forms.FocusEventArgs e)
        {
            try
            {
                var x = (TimePicker)sender;
                var yy = x.Time;
                DateTime time = DateTime.Today.Add(yy);
                //string displayTime = time.ToString("hh:mm tt");

                //DateTime dateTime1 = DateTime.ParseExact(yy.ToString(@"hh\:mm"), "hh:mm tt", CultureInfo.InvariantCulture);
                TextEntry.Text = time.ToString("hh:mm tt", CultureInfo.InvariantCulture);
            }
            catch (Exception ex)
            {
            ...
            //SelectedDate = sender.;
            //TextEntry.Text = String.IsNullOrEmpty(Format) ? e.NewDate.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture) : e.NewDate.ToString();
        }

        void TextEntry_Focused(System.Object sender, Xamarin.Forms.FocusEventArgs e)
        {
            TextEntry.Unfocus();
        ...
        void dropOffTime_Unfocused(System.Object sender, Xamarin.Forms.FocusEventArgs e)
        {
            try
            {
                var x = (TimePicker)sender;
                ...
                //DateTime dateTime1 = DateTime.ParseExact(yy.ToString(@"hh\:mm"), "hh:mm tt", CultureInfo.InvariantCulture);
                TextEntrydropOff.Text = time.ToString("hh:mm tt", CultureInfo.InvariantCulture);
            }
            catch (Exception ex)
            {
        ...
        void TextEntry_FocuseddropOff(System.Object sender, Xamarin.Forms.FocusEventArgs e)
        {
            TextEntrydropOff.Unfocus();
            Device.BeginInvokeOnMainThread(() => {
                dropOfftime.Focus();
            });
        }

        void TextEntrypickUpLocation_Focused(System.Object sender, Xamarin.Forms.FocusEventArgs e)
        {
            TEpickUpLocation.Unfocus();
            Device.BeginInvokeOnMainThread(() => {
                pickUpLocation.Focus();
            });
        ...
        void TextEntrydropoffLocation_Focused(System.Object sender, Xamarin.Forms.FocusEventArgs e)
        {
            TEdropOffLocation.Unfocus();
            Device.BeginInvokeOnMainThread(() => {
                dropOffLocation.Focus();
            });
        ...
        private void dropoffLocation_Unfocused(object sender, FocusEventArgs e)
        {
            if (dropOffLocation.SelectedIndex != -1)
            {
                TEdropOffLocation.Text = dropOffLocation.SelectedItem.ToString();
            }
        }
